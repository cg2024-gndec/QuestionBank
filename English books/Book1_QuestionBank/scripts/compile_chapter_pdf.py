...
Description: ReportLab PDF compiler for Book 1 (Class I English) Question Banks.
             Parses Markdown tags and compiles a beautifully styled, professional
...
class NumberedCanvas(canvas.Canvas):
    ...
    def draw_page_decorations(self, page_count):
        ...
        if self._pageNumber > 1:
            self.drawString(36, 762, "CBSE / ICSE Class I English -- Question Bank")
...
def build_pdf(chapter_num):
    ...
    story = []
    story.append(Paragraph(f"CBSE / ICSE CLASS I ENGLISH QUESTION BANK", custom_styles['DocTitle']))
    story.append(Paragraph(f"CHAPTER {chapter_num} (300 QUESTIONS)", custom_styles['DocSubtitle']))
    ...
    for cat_title, filename in files_order:
        ...
        story.append(Paragraph(cat_title, custom_styles['H1']))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--chapter", help="Chapter number e.g. 01")
    parser.add_argument("--all", action="store_true", help="Compile all 15 chapters")
    args = parser.parse_args()

    if args.all:
        print("Starting PDF Compilation for all 15 Chapters of Book 1...")
        for i in range(1, 16):
            ch_num = f"{i:02d}"
            print(f"Processing chapter {ch_num}...")
            build_pdf(ch_num)
        print("=== Compilation Finished: 15/15 successful ===")
    elif args.chapter:
        build_pdf(args.chapter)
    else:
        print("Specify --chapter XX or --all")
